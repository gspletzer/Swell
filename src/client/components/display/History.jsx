...
class History extends Component {
  ...
  addHistoryToNewRequest() {
    const requestFieldObj = {
      method: this.props.content.request.method
        ? this.props.content.request.method
        : "GET",
      protocol: this.props.content.protocol
        ? this.props.content.protocol
        : "http://",
    ...
    if (this.props.content.request.headers) {
      headerDeeperCopy = JSON.parse(
        JSON.stringify(this.props.content.request.headers)
      );
    ...
    if (
      this.props.content.request.cookies &&
      !/ws/.test(this.props.content.protocol)
    ) {
      cookieDeeperCopy = JSON.parse(
        JSON.stringify(this.props.content.request.cookies)
      );
    ...
    const requestBodyObj = {
      bodyType: this.props.content.request.bodyType
        ? this.props.content.request.bodyType
        : "raw",
      bodyContent: this.props.content.request.body
        ? this.props.content.request.body
        : "",
      bodyVariables: this.props.content.request.bodyVariables
        ? this.props.content.request.bodyVariables
        : "",
      rawType: this.props.content.request.rawType
        ? this.props.content.request.rawType
        : "Text (text/plain)",
      JSONFormatted: this.props.content.request.JSONFormatted
        ? this.props.content.request.JSONFormatted
        : true,
    };
    ...
    if (this.props.content && this.props.content.gRPC) {
      const streamsDeepCopy = JSON.parse(
        JSON.stringify(this.props.content.streamsArr)
      );
      const contentsDeepCopy = JSON.parse(
        JSON.stringify(this.props.content.streamContent)
      );
      ...
      this.setState(
        ...
        () => {
          ...
          document.getElementById(
            "stream"
          ).innerText = this.props.content.request.method;
        }
      );
    }
  }
    ...
    return (
      <div className={"history-container"} onClick={this.props.focusOnForm}>
        <div
          ...
          onClick={this.addHistoryToNewRequest}
        >
          <div className={"history-method"}>
            {this.props.content.request.method}
          </div>
          <div className={"history-url"}> {this.props.content.url}</div>
        </div>
        <div className="history-delete-container">
          <div className="history-delete-fade"></div>
          <div className={"history-delete-button"} onClick={this.deleteHistory}>
            <img
              className="history-delete-image"
              src={Trashcan}
